...
The code to reproduce this report is available [on Github](https://github.com/williamxcheng/stat-471-final-project).

\newpage

# Executive Summary

**Problem.** Despite the historical body of research focused on understanding the factors that contribute to vaccine hesitancy, the COVID-19 vaccination campaign differs from past vaccination drives in that it is the largest vaccination drive in history, and the vaccine is one of the fastest developed in history. There is still much to be learned about population-level demographic and socioeconomic factors that make some populations more resistant to getting vaccinated. Hence, for my final project, I decided to analyze which factors were most predictive of vaccine hesitancy from the period of September 29 - October 11, 2021. While only analyzing data from this period inherently limits our results, we also note that recent data may actually be most likely to reflect differences in vaccine hesitancy, as this was the period when vaccines were made available to the U.S. public and when the efficacy and safety of the vaccines had been tested and released.
...
**Significance.** I hope that my analysis will contribute to the growing body of research on factors contributing to COVID-19 vaccine hesitancy, and I hope that they can be used to improve the efficacy of future vaccine rollouts. My results highlight the importance of analyzing sociol, economic, health, and lifestyle factors in efforts to understand vaccine hesitancy.

# Data

## Data sources

My dataset originated from the Household Pulse Survey conducted by the U.S. Census. The Household Pulse Survey seeks to measure how the COVID-19 pandemic is impacting households across the country from a social and economic perspective. Specifically, along with measuring the intention of the respondent to get vaccinated for COVID-19, the Household Pulse Survey includes measures of demographics, economic welfare, and lifestyle factors. To construct my dataset, I pulled data from one collection period, amounting to the timeframe between September 29 to October 11, 2021. 
...
For the first model, I decided to fit a logistic regression with all 62 features to the training data. For this model, Table \@ref(tab:logistic-confusion-matrix) shows the resulting confusion matrix. The logistic regression revealed that the following variables are significantly associated with the response: [], []. The misclassification error as seen in Table \@ref(tab:logistic-evaluation) shows that the model has a misclassification rate of []. The resulting false positive rate is [] and the false negative rate is [], which shows that the model tends to [].
...
While the logistic regression seemed to produce a model that worked well, fitting a logstic model with so many explanatory variables might incur a large cost in variance and lead to suboptimal predictions. [CHANGE] Hence, I decided to build and evaluate shrinkage models with the hopes of getting a more parsimonious and interpretable model.[CHANGE] I ran three cross-validated regressions for which optimal values of lambda were chosen according to the one-standard-error rule: ridge regression, LASSO (Least Absolute Shrinkage and Selection Operator) regression, and elastic net regression. 

For the ridge regression, Figure \@ref(fig:ridge-CV-plot) shows the CV plot, Figure \@ref(fig:ridge-trace-plot) shows the trace plot, and [TODO] Table \@ref(tab:ridge-coefficients) shows the selected features and their coefficients. Table \@ref(tab:ridge-evaluation) [] shows that the model has a misclassification rate of [].
...
- *Demographics*
  ...
  - Age (`age`): Denotes age of individual. This is the only feature that is numerical.  
  
**Economic factors** 

- *Household*
  - Kids (`kids`): Indicates having kids in household (defined as 18 years or younger).
  - Number of kids (`number_kids_household`): Number of kids in household (defined as 18 years or younger).
  - Number of adults (`number_adults_household`): Number of adults in household (defined as 18 years or older).
